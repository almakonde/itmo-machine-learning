...
DATASET = 'SPAM'  # SPAM or NEWS
VECTORIZER = 'COUNT'  # COUNT, HASH or TFIDF
...
def main():
    ...
    results = classify(X_train, X_test, y_train, y_test, feature_names, target_names)
...
def classify(X_train, X_test, y_train, y_test, feature_names, target_names):
    results = []
    # Train sparse Naive Bayes classifiers
    print('=' * 80)
    print("Naive Bayes")
    results.append(benchmark(MultinomialNB(alpha=.01), X_train, X_test, y_train, y_test, feature_names, target_names))
    results.append(benchmark(BernoulliNB(alpha=.01), X_train, X_test, y_train, y_test, feature_names, target_names))
    if DATASET == 'SPAM':  # 'NEWS' will raise MemoryError
        results.append(benchmark(GaussianNB(), X_train, X_test, y_train, y_test, feature_names, target_names))
        ...
        for clf, name in (
            ...
            print('=' * 80)
            print(name)
            results.append(benchmark(clf, X_train, X_test, y_train, y_test, feature_names, target_names))
...
def benchmark(classifier, X_train, X_test, y_train, y_test, feature_names, target_names):
    ...
    for n_folds in [2, 3, 4, 5]:
        ...
        print("Cross validation [n_folds=%d] Accuracy: %0.2f (+/- %0.2f)" % (
            n_folds, scores.mean(), scores.std()
        ))
